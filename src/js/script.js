...
$(function () {
    ...
    var event = (1538233200 - new Date() / 1000) < 0 && (1538276400 - new Date() / 1000) > 0; // 17 сентября 2018 года, 0:00:00
    var eventHeader = "Муз.посвят ФИВТ";
    var eventHTML = "Начинается сезон посвятов. Первокуры празднуют и Физтех.Радио празднует вместе с ними в нашей новой рубрике муз. посвят.<br><br>" +
        "Первыми пройти ритуал посвящения и постигнуть таинства общажных вечеринок предстоит первокурам факультета инноваций и высоких технологий.<br>Поздравляем всех новоиспеченных ФИВТов и желаем им поменьше багов и побольше безошибочных компиляций.<br><br>" +
        "По вашим заявкам мы загрузили специальный плейлист, подготовили пару сюрпризов и сегодня эта музыка играет для вас!";


    if (event) {
        $('#bottom-header').html(eventHeader);
        $('#bottom-text').html(eventHTML);
    }
    else {
    ...
    $('#bottom').click(function () {
        ...
        bottomText.toggleClass('open');
        $('#bottom-text a').toggleClass('open');
        $('#bottom-text table').toggleClass('open');
        $('#bottom-text tbody').toggleClass('open');
        $('#bottom-text tr').toggleClass('open');
        $('#bottom-text td').toggleClass('open');

        if ($(this).hasClass('open')) {
            bottomClickCounter++;
            if (!event)
            {
                if (bottomClickCounter > 2) {
                    bottomText.html("Че ты клацаешь? Новая шутка будет завтра, это же daily jokes!");
                }
                else {
